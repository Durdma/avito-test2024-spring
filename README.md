...
6. Некоторые замечания по взаимодействию с API:
  ...
  7. Были добавлены метрики с использованием prometheus. Их можно получить по ``localhost:9090``. Были реализованы 2 кастомные метрики: ``http_request_duration_seconds`` и ``response_status``. Они были добавлены
